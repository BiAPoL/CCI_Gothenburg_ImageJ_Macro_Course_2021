...
for (i = 0; i < lengthOf(files); i++) {
	...
	if (endsWith(filename, "d.tif")) {
		...
		setResult("Filename", nResults() - 1, filename);
		setResult("CLIJ count", nResults() - 1, number_of_objects_clij);
		setResult("StarDist count", nResults() - 1, number_of_objects_stardist);
		setResult("Threshold Watershet count", nResults() - 1, number_of_objects_threshold_watershed);
	}	
}
// just make the table look nicer
